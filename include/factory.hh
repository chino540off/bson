...
{
	private:
		typedef ManufacturedType* (*BaseCreateFn)(std::vector<std::uint8_t> const &, unsigned int &);
	...
	public:
		...
		void RegCreateFn(ClassIDKey const & className, BaseCreateFn fn)
		{
								  ...
								  unsigned int &						pos) const
		{
			...
			if (reg != registry.end())
				return registry.at(className)(buffer, pos);
			else
				return nullptr;
		}
};
...
{
	public:
		static AncestorType * CreateInstance(std::vector<std::uint8_t> const &	buffer,
											 unsigned int &						pos)
		{
			return new ManufacturedType(buffer, pos);
		}
